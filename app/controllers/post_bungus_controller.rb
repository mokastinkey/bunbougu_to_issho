class PostBungusController < ApplicationController
  ...
  def index
    ...
    @genres = Genre.all
    # ランキング
    @month_like_ranks = PostBungu.find(Like.group(:post_bungu_id).where(created_at: Time.current.all_month).order('count(post_bungu_id) desc').limit(3).pluck(:post_bungu_id))
  end
